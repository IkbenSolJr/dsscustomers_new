...
class dsscustomers(models.Model):
    ...
    thuphidt = fields.Boolean(string='Phí dào tạo')
    thuphinghe = fields.Boolean(string='Phí Nghề')
    ...
    def expand_tienganh(self):
        ...
        self.ensure_one()  # Đảm bảo chỉ làm việc với 1 bản ghi
        ...
        return {
            ...
            'view_mode': 'tree',  # Mở tree view trước, có thể mở form view khi click
            'domain': [('sinhvien_ids', '=', self.id)],  # Chỉ hiển thị dữ liệu liên quan
            # 'target': 'new',  # Hiển thị trong cùng cửa sổ
            'target': 'current',  # Hiển thị trong cùng cửa sổ
        }
